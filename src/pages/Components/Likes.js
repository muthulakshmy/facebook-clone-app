...
const Saved = () => {
  ...
  const [bookmarkedPosts, setBookmarkedPosts] = useState(posts.filter((post) => post.liked));
  ...
  return (
    <Box sx={{mt:10}}>
      ...
      {bookmarkedPosts.length ? (
              ...
              <CardHeader
                ...
                action={
                  <IconButton onClick={() => handleRemoveFromSaved(post.id)}>
                    <ThumbUpAltRounded sx={{color:"#17A9FD"}} />
                  </IconButton>
                }
              />
              <Divider />
        ...
        <Typography variant="h6" sx={{textAlign:"center",mt:35}}>No Liked posts yet.</Typography>
      )}
    </Box>
  );
};
